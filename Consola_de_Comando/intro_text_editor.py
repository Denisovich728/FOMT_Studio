...
class IntroTextEditorWidget(QWidget):
    ...
    def __init__(self, project, parent=None):
        ...
        self.mapping_path = os.path.join(
            ...
            "Banco_de_Datos", "Cilixes", "fomt", "Fomt_Pointer_mapping_fomt.csv"
        )
        self.mappings = []
        self.setup_ui()
        self.load_mapping()
    ...
    def setup_ui(self):
        ...
        # Toolbar
        toolbar = QHBoxLayout()
        lbl = QLabel(tr("intro_editor_title", lang))
        
        btn_refresh = QPushButton(tr("btn_refresh", lang))
        btn_refresh.clicked.connect(self.load_mapping)
        
        btn_save = QPushButton(tr("btn_save_all", lang))
        ...
        self.table.setHorizontalHeaderLabels([
            tr("col_orig_pointer", lang),
            tr("col_decorated_text", lang),
            tr("col_references", lang)
        ])
    ...
    def load_mapping(self):
        ...
        if not os.path.exists(self.mapping_path):
            QMessageBox.warning(
                self, 
                tr("msg_error", lang), 
                tr("err_mapping_not_found", lang).format(path=self.mapping_path)
            )
            return
        ...
        try:
            ...
            QMessageBox.critical(
                self, 
                tr("msg_error", lang), 
                tr("err_loading_csv", lang).format(error=str(e))
            )
            return
    ...
    def save_all(self):
        ...
        if cambios > 0:
            ...
            QMessageBox.information(
                self, 
                tr("msg_success", lang), 
                tr("msg_repoint_success", lang).format(count=cambios)
            )
            self.refresh_table()
        else:
            QMessageBox.information(
