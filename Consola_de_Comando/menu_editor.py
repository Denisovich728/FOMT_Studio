...
class MenuEditorWidget(QWidget):
    ...
    def _init_ui(self):
        lang = get_lang_from_widget(self)
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel(tr("menu_editor_title", lang)))
        layout.addWidget(QLabel(tr("menu_editor_desc", lang)))

        self.table = QTableWidget()
        self.table.setColumnCount(5)
        self.table.setHorizontalHeaderLabels([
            tr("col_ptr_offset", lang),
            tr("col_category", lang),
            tr("col_function", lang),
            tr("col_real_addr", lang),
            tr("col_text_unlimited", lang)
        ])
    ...
    def inject_script_at_cursor(self):
        ...
        if row == -1:
            QMessageBox.warning(self, tr("msg_error", lang), tr("msg_select_row", lang))
            return
    ...
    def save_all(self):
        ...
        if count > 0:
            QMessageBox.information(
                self, 
                tr("title_sync_success", lang), 
                tr("msg_sync_success", lang).format(count=count)
            )
            ...
            QMessageBox.warning(
                self, 
                tr("title_no_changes", lang), 
                tr("msg_no_changes_texts", lang)
            )
