...
pointers = {}
with open(r'j:\Repositorios\fomt_studio\Nucleos_de_Procesamiento\Cilixes\fomt\Fomt_Sprite_data.csv', 'r') as f:
    reader = csv.reader(f)
    next(reader)
    for row in reader:
...
animations = []
with open(r'j:\Repositorios\fomt_studio\Nucleos_de_Procesamiento\Cilixes\fomt\Fomt_Animations.csv', 'r') as f:
    reader = csv.reader(f)
    next(reader)
    for row in reader:
        name = row[0]
...
for anim_id, name in animations:
    ...
    for c in cmds:
        if c['type'] == 'draw':
            ...
            # If the user wants dynamically loaded graphics, we don't need to add arg2 for offset unless it's requested.
            # But the original code added arg2 to r0_lower/r0_upper. 
            # If it's working without it, let's keep it simple.
            ...
            if current_frame_img:
                ...
                # Save unique frames for sprite sheet
