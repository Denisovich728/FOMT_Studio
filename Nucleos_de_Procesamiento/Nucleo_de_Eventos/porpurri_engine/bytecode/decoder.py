...
def decode_code_chunk(code_data: bytes) -> bytes:
    ...
    if head_size != len(code_data) - 4:
        raise DecodeError("BadCodeChunk")
        
    code_with_nops = code_data[4:]
    real_end = len(code_with_nops) - 1
    while real_end > 0 and code_with_nops[real_end] != OPCODE_END:
        real_end -= 1
        
    return code_with_nops[:real_end]
