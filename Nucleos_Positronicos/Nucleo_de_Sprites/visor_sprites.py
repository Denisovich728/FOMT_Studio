...
import io
import sys
from Banco_de_Datos.Utilidades.rutas import get_data_path
from Consola_de_Comando.i18n import tr, get_lang_from_widget
...
class SpriteInfoPanel(QFrame):
    def tr(self, key):
        return tr(key, get_lang_from_widget(self))
    ...
    def _build_ui(self):
        ...
        title = QLabel(self.tr("sprite_info_title"))
        ...
        self.lbl_name = self._field(self.tr("field_name_label"))
        self.lbl_offset = self._field(self.tr("field_tile_offset"))
        self.lbl_palette = self._field(self.tr("field_palette_offset"))
        self.lbl_size = self._field(self.tr("field_data_size"))
        self.lbl_dims = self._field(self.tr("field_dimensions"))
        self.lbl_category = self._field(self.tr("field_category"))
        ...
        layout.addWidget(QLabel(self.tr("palette_label"), styleSheet="color: #FFD700; font-size: 11px; font-weight: bold; letter-spacing: 1px;"))
    ...
    def update_info(self, name, tile_off, pal_off, data_size, dims, category, palette_colors=None):
        self.lbl_name.setText(f"{self.tr('field_name_label')} {name}")
        self.lbl_offset.setText(f"{self.tr('field_tile_offset')} 0x{tile_off:06X}")
        self.lbl_palette.setText(f"{self.tr('field_palette_offset')} 0x{pal_off:06X}")
        self.lbl_size.setText(f"{self.tr('field_data_size')} {data_size} bytes")
        self.lbl_dims.setText(f"{self.tr('field_dimensions')} {dims}")
        cat_colors = {"Overworld": "#00FF96", "Portrait": "#00BFFF", "Animal": "#FFD700"}
        c = cat_colors.get(category, "#AAAAAA")
        self.lbl_category.setText(f"{self.tr('field_category')} {category}")
...
class VisorSprites(QWidget):
    ...
    def _init_ui(self):
        ...
        # ── Header ──
        header = QLabel("🎨 " + self.tr("sprite_explorer_title"))
        header.setStyleSheet("font-size: 18px; font-weight: bold; color: #00FF96;")
        layout.addWidget(header)

        # ── Filtros pill-buttons (estilo visor de audio) ──
        filter_layout = QHBoxLayout()
        filter_label = QLabel(self.tr("filter_label"))
        filter_label.setStyleSheet("color: #888888; font-size: 11px;")
        filter_layout.addWidget(filter_label)

        self.filter_buttons = {}
        filter_defs = [
            ("ALL",       self.tr("filter_all"),              "#FFFFFF"),
            ("Overworld", self.tr("filter_overworld"),     "#00FF96"),
            ("Portrait",  self.tr("filter_portrait"),      "#00BFFF"),
            ("Animal",    self.tr("filter_animal"),        "#FFD700"),
        ]
        ...
        self.lbl_preview = QLabel(self.tr("select_sprite_tip"))
        ...
        ctrl_layout.addWidget(QLabel(self.tr("zoom_label"), styleSheet="color: #888; font-size: 11px;"))
        ...
        ctrl_layout.addWidget(QLabel(self.tr("tiles_row_label"), styleSheet="color: #888; font-size: 11px;"))
        ...
        self.btn_export_bmp = QPushButton(self.tr("btn_export_bmp"))
        ...
        self.btn_export_gif = QPushButton(self.tr("btn_export_gif"))
        ...
        self.btn_export_all = QPushButton(self.tr("btn_export_all"))
    ...
    def reset(self):
        ...
        self.lbl_preview.setText(self.tr("select_sprite_tip"))
    ...
    def _render_sprite(self, sp):
        if not self.project or not self.project.base_rom_data:
            self.lbl_preview.setText(self.tr("err_no_rom_loaded"))
            return
        ...
        if img:
            ...
            self.lbl_preview.setText(self.tr("err_render_sprite"))
            self._current_image = None
    ...
    def _on_export_bmp(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, self.tr("export_bmp_title"), f"{sp['name']}.bmp", "BMP (*.bmp)")
        if path:
    ...
    def _on_export_gif(self):
        ...
        from Nucleos_Positronicos.Nucleo_de_Sprites.sprite_decoder import SpriteRenderer
        from Nucleos_Positronicos.Nucleo_de_Mapas.codec_tiles import bgr555_to_rgb
        from Nucleos_Positronicos.Nucleo_de_Mapas.mapas import decompress_lz77
        ...
        if frames:
            path, _ = QFileDialog.getSaveFileName(self, self.tr("export_gif_title"), f"{sp['name']}.gif", "GIF (*.gif)")
            if path:
    ...
    def _on_export_all(self):
        if not self._sprites or not self.project:
            return
        folder = QFileDialog.getExistingDirectory(self, self.tr("export_folder_title"))
        if not folder:
            return
        ...
        self.count_label.setText(self.tr("msg_exported_sprites").format(count=count, folder=folder))
