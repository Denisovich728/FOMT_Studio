...
class PaletteEditorDialog(QDialog):
    ...
    def __init__(self, palette_colors, parent=None):
        ...
        """
        super().__init__(parent)
        self.setWindowTitle(self.tr("palette_editor_title"))
        self.setModal(True)
        self.resize(680, 520)
    ...
    def _build_ui(self):
        ...
        lbl_picker = QLabel("🎨  " + self.tr("color_selector"))
        ...
        lbl_prev = QLabel(self.tr("color_previous"))
        ...
        lbl_new = QLabel(self.tr("color_new"))
        ...
        # ── HEX Input ─────────────────────────────────────────────────────
        hex_row = QHBoxLayout()
        lbl_hex = QLabel(self.tr("hex_label"))
        ...
        lbl_gba = QLabel(self.tr("gba_label"))
        ...
        btn_apply = QPushButton(self.tr("btn_apply_slot"))
        ...
        # ── Accept / Cancel ────────────────────────────────────────────────
        btn_row = QHBoxLayout()
        btn_ok = QPushButton(self.tr("btn_save_palette"))
    ...
    def _select_slot(self, idx):
        ...
        self.lbl_slot_info.setText(self.tr("slot_selected_info").format(idx=idx))
    ...
    def _copy_hex(self):
        ...
        QMessageBox.information(self, self.tr("msg_copied"), self.tr("msg_copied_hex"))
    ...
    def _paste_hex(self):
        ...
        if not matches:
            QMessageBox.warning(self, self.tr("msg_error"), self.tr("msg_no_hex"))
            return
            
        if len(matches) < 16:
            QMessageBox.warning(self, self.tr("msg_warning"), self.tr("msg_warning_partial").format(count=len(matches)))
        ...
        self._select_slot(1)
        self.palette_changed_live.emit(list(self._palette))
        QMessageBox.information(self, self.tr("msg_pasted"), self.tr("msg_pasted_palette"))
    ...
    def _import_pal(self):
        ...
        try:
            ...
            self._select_slot(1)
            self.palette_changed_live.emit(list(self._palette))
            QMessageBox.information(self, self.tr("msg_imported"), self.tr("msg_imported_palette"))
            
        except Exception as e:
            QMessageBox.critical(self, self.tr("msg_error"), f"{self.tr('msg_imported_palette')}: {e}")
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                ...
                for r, g, b in self._palette:
                    rq, gq, bq = quantize_to_gba(r, g, b)
                    f.write(f"{rq} {gq} {bq}\n")
            QMessageBox.information(self, self.tr("msg_exported"), self.tr("msg_exported_palette").format(path=path))
        except Exception as e:
            QMessageBox.critical(self, self.tr("msg_error"), f"{self.tr('msg_exported_palette').format(path='')}: {e}")
