...
Basado en ingeniería inversa directa de BlueSpider (mapped.pyd, mapdata.pyd):
  ...
  • LZ77 decompressor (header 0x10) y ArcanoMarvelous RLE (header 0x70)
"""
import struct
import zlib
...
class HuffLZDecompressor:
    ...
    def decompress(self):
        ...
        if huffType == 0:
            ...
            self.read_huffman_tree(4)
            def read_byte(): return self.read_byte_huff4()
            self.read_byte = read_byte
        elif huffType == 2:
        ...
        if compType == 0:
            ...
            self.read_lz_lookup_ladder(7)
            while len(self.out) < targetSize:
                if self.reader.read_bit() == 0:
                    self.out.append(self.read_byte())
                else:
                    i = self.reader.read_bits(3)
                    if i < 7:
                        ladderItem = self.lz_ladder[i]
                        distanceBits = ladderItem['bits']
                        offset = ladderItem['offset']
                        distance = self.reader.read_bits(distanceBits) + offset
                        length = self.reader.read_bits(4) + 3
                        self.apply_lz_bytes(distance, length)
                    elif i == 7:
                        count = 0
            ...
            while len(self.out) < targetSize:
                if self.reader.read_bit() == 0:
                    ...
                    if i < 3:
                        ...
                        offset = ladderItem['offset']
                        distance = self.reader.read_bits(distanceBits) + offset
                        length = self.reader.read_bits(3) + 2
                        self.apply_lz_bytes(distance, length)
                    elif i == 3:
                        count = 0
                        value = 0
                        ...
                        if self.reader.read_bit() == 0:
                            ...
                            raise Exception("compType 3 unexpected sequence")
        elif compType == 4:
            while len(self.out) < targetSize:
                self.out.append(self.read_byte())
        else:
            raise ValueError(f"Unknown compType: {compType}")

        # 3. Post-decompression filters
        if filtType == 1:
            r5 = 0
            for i in range(2, len(self.out) - 1, 2):
                r2 = self.out[i] | (self.out[i+1] << 8)
                r1 = (r2 >> 4)
                r3 = (r2 >> 12)
                r4 = (r2 >> 8)
                r1 = (r1 + r5)
                ...
                r5 &= 0xf
                r1_shifted = r1 << 4
                r3_shifted = (r3 & 0xf) << 12
                r4_shifted = r5 << 8
                r1_out = r2 | r1_shifted | r3_shifted | r4_shifted
                self.out[i] = r1_out & 0xFF
                self.out[i+1] = (r1_out >> 8) & 0xFF
        elif filtType == 2:
            r4 = 0
            for i in range(2, len(self.out) - 1, 2):
                r2_val = self.out[i] | (self.out[i+1] << 8)
                r3 = r2_val >> 8
                r2 = (r2_val + r4) & 0xFF
                r4 = (r3 + r2) & 0xFF
                self.out[i] = r2
        ...
        return bytes(self.out[:targetSize])

    def read_huffman_tree(self, bitSize):
        self.huffman = {
            'left': None,
            'right': None,
            'leaf': False
        ...
        def read_node(node):
            if self.reader.read_bit() == 0:
                node['left'] = {'left': None, 'right': None, 'leaf': False}
                read_node(node['left'])
                node['right'] = {'left': None, 'right': None, 'leaf': False}
                read_node(node['right'])
            else:
                node['leaf'] = True
                node['value'] = self.reader.read_bits(bitSize)
        read_node(self.huffman)
        
    def read_byte_huff4(self):
        val = 0
        for i in range(2):
            node = self.huffman
            while not node['leaf']:
    ...
    def read_lz_lookup_ladder(self, count):
        self.lz_ladder = []
        for _ in range(count):
            bits = self.reader.read_bits(4)
            offset = 1 if bits == 0 else (1 << bits) + 1
            self.lz_ladder.append({
                'bits': bits,
                'offset': offset
            })
...
def decompress_unicorn(data, offset):
    ...
    # Search for the true ArcanoMarvelous Decompressor routine (push {r4-r7, lr}; mov r4, r8; mov r5, sb...)
...
def decompress_arcano_marvelous(data: bytes, offset: int) -> bytearray:
    """
    Descomprime datos comprimidos con el algoritmo "ArcanoMarvelous" (0x70).
...
def decompress_auto(data: bytes, offset: int) -> bytes:
    """Detecta automáticamente LZ77 o ArcanoMarvelous y descomprime."""
    if offset >= len(data):
        return b''
    header = data[offset]
    if header == 0x10:
        ...
        return decompress_arcano_marvelous(data, offset)
    elif header == 0x00:
...
def compress_arcano_marvelous(data: bytes) -> bytes:
    """
...
class MapHeader:
    ...
    def save_behavior_dict_to_rom(self, project, dict_bytes: bytes) -> bool:
        """
        Guarda el diccionario de comportamientos en la ROM.
        Si es más grande que el original, repuntea a espacio libre.
        """
        old_ptr = self.p_obj1 & 0x01FFFFFF
        limit = (self.p_obj2 & 0x01FFFFFF) - old_ptr
        
        # Validar si el diccionario original estaba justo antes de p_obj2
        if old_ptr == 0 or (self.p_obj2 & 0x01FFFFFF) < old_ptr:
            # Fallback seguro: Asumimos que no hay límite conocido, repuntear siempre
            limit = -1
            
        if len(dict_bytes) <= limit and limit > 0:
            # Cabe en su sitio original
            project.write_patch(old_ptr, dict_bytes)
        else:
            # Repuntear (buscar 1024 bytes o lo que mida el dict nuevo)
            new_ptr = project.allocate_free_space(len(dict_bytes))
            if not new_ptr:
                return False
            project.write_patch(new_ptr, dict_bytes)
            self.p_obj1 = new_ptr | 0x08000000
            
            # Guardar el nuevo puntero en el header del mapa (offset + 24)
            ptr_bytes = struct.pack('<I', self.p_obj1)
            project.write_patch(self.offset + 24, ptr_bytes)
...
class MapParser:
    ...
    def save_map(self, map_index: int, renderer) -> bool:
        ...
        # 2. Comprimir con el algoritmo nativo de FoMT (ArcanoMarvelous RLE - 0x70)
        bg3_comp = compress_arcano_marvelous(bg3_raw)
        bg2_comp = compress_arcano_marvelous(bg2_raw)
        bg1_comp = compress_arcano_marvelous(bg1_raw)
